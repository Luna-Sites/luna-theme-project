...
PROJECT_NAME=luna-theme
STACK_NAME=luna-theme-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://luna-theme.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t Luna-Sites/luna-theme-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t Luna-Sites/luna-theme-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name luna-theme-frontend-acceptance --link luna-theme-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d Luna-Sites/luna-theme-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name luna-theme-backend-acceptance -d Luna-Sites/luna-theme-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop luna-theme-frontend-acceptance
	@docker stop luna-theme-backend-acceptance
